...
bool ewl::Pixmap::loadFromTIFF(const char* path) {
    ...
    if(tiff) {
        ...
        pixels = (ewl::uint8_t*)malloc(pix_size);

        if(TIFFReadRGBAImage(tiff,this->size_x, this->size_y, (ewl::uint32_t*)this->pixels)) {
            for(ewl::uint32_t x = 0; x < this->size_x; x++) {
                for(ewl::uint32_t y = 0; y <= this->size_y/2; y++) {
                    
                    ewl::Pixel tmprgb = getPixelX(y*this->size_x+x);
...
bool ewl::Pixmap::exportToTIFF(const char* path) const {
    if(isLegit()) {
        TIFF* f = TIFFOpen(path,"w");
        if(f) {
            ewl::uint16_t samples = 3;
            ...
            ewl::Pixmap px = constructDecreased(samples);

            for(ewl::uint32_t y = 0; y < size_y; y++) {
